...
export async function LandingPage() {
  ...
  return (
    <main className={styles.page} data-theme={config.theme} style={{ "--landing-accent": config.accent, "--landing-accent-fg": melhorFrenteSobre(config.accent) } as CSSProperties}>
      ...
      <section id="planos" className={styles.section}><p className={styles.eyebrow}>CRESÇA NO SEU RITMO</p><h2>{config.pricing_title}</h2><p className={styles.priceNote}>{config.pricing_note}</p><div className={styles.prices}>{config.plans.map((plan, index) => <article className={styles.plan} data-featured={index === 1} key={plan.slug}><p className={styles.planLabel}>{index === 1 ? "MAIS POSSIBILIDADES" : "SEU PRÓXIMO PASSO"}</p><h3>{plan.name}</h3><p>{plan.description}</p><strong className={styles.price}>{new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(plan.price_cents / 100)}/mês</strong><small>Cobrança mensal recorrente</small><ul>{plan.features.map((feature, i) => <li key={i}><span>✓</span>{feature}</li>)}</ul><PlanCta name={plan.name} destination={plan.checkout_url} /></article>)}</div></section>
      ...
      <section id="proximo-passo" className={styles.closing}><p className={styles.eyebrow}>CONSTRUA SUA PRÓXIMA ETAPA</p><h2>{config.closing_title}</h2><p>{config.closing_description}</p><a className={styles.primary} href="#planos">Escolher meu plano ↗</a><p className={styles.fine}>Já possui acesso? <a href="/login">Entre na sua organização →</a></p></section>
